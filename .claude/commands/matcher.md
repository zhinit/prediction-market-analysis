...
- The Poly YES side (what the websocket orderbook quotes) is the `marketSides` entry with `"long": true` — match it to the Kalshi sub-market by `team.name`. Verified empirically across MLB, NPB, and tennis on 2026-07-11. `team.ordering` (home/away) and slug order do NOT indicate the YES side. Each pair of same-slug match entries must have one `kalshi_yes_eq_poly_yes` and one `kalshi_yes_eq_poly_no` — two identical directions on a pair is always a bug.
